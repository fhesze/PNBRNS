...
# UNITS: muM
 variables := [cmm,cbst,cmminh,cd,cc] ;
...
term1:=qmax/kexch;
term22:=cbst+km+term1;

rate1 := qmax*cmm/(km+cmm) ;
rate2 := kexch/2.0*term22*(1.0-(1.0-4.0*cbst*term1/(term22**2.0))**0.5);
rate3 := qmax*cmminh/(km+cmminh) * kinhib / (kinhib + cmminh) ;
rate4 := kexch * ( cd - cc ) ;
rate5 := qmax * cc / ( cc + km ) * kinhib / (kinhib + cc) ;
...
bio_name := [km,mumax,Diff,radi,robac,kinhib] ;
bio_val :=[0.261e-6,3.26e-8,6.0e-10,radi,robac,kinhib] ; 
...
nparameters := 3 ;
parameterslist := [radi,robac,kinhib] ;
# 
#   
# Switches
