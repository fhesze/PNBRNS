...
robac           = robacexp*Avexp/Av_model;                                 % To make a new bac density for hetrogenos medium in order to have similar conditions to homogenous case
...
path            = getLibPath('unix', 'Scenario III');
lib_file        = [path 'brns.so'];
lib_header      = [path 'brnsdllRateReturn.h'];
...
for pore_i=1:pores_no
        ...
        % Calling BRNS
        calllib('brns','invokebrns_',cat_ptr, cit_ptr, co_ptr, nos_ptr, ts_ptr, fcb_ptr, rv_ptr, posx_ptr, posy_ptr, posz_ptr, p_ptr, ws_ptr, par_ptr,rrv_ptr);
